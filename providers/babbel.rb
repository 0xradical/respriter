...
BABBEL   = File.join(ROOT, 'providers', 'babbel')
SEEDS    = File.join(ROOT, 'db', 'seeds', 'babbel')
template = File.read(File.join(BABBEL, 'setup.sql.erb'))

def sql_safe_parsed_unparsed_code(path)
  code = File.read(File.join(BABBEL, path))
  Unparser.parse(code)
  '"' + code.gsub('\\', '\&\&').gsub(/\n/, '\n').gsub("'", "''").gsub('"', '\"') + '"'
end
...
sitemap_url = 'https://www.babbel.com/sitemap/classes/1'
pricing_url = 'https://www.babbel.com/premium'
