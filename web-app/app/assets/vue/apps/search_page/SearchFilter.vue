      ...
      <div class="mobile-filter-wrapper">
        <transition-group name="mobile-filter" tag="div">
          <div
            v-show="
              !(
                isFiltering.root_audio ||
                isFiltering.subtitles ||
                isFiltering.provider_name ||
                isFiltering.price
              )
            "
            class="mobile-filter-view mobile-filter-facets-view"
            key="mobile-filter-facets"
          >
            <div
              @click="isFiltering.root_audio = true"
              class="mobile-filter-facets__facet"
            >
              <search-fieldset
                :title="$t('dictionary.audios')"
                :subtitle="filter.root_audio.map(audioValue).join(', ')"
                :stand-out="filter.root_audio.length > 0"
              >
                <template #action>
            ...
            <div
              @click="isFiltering.subtitles = true"
              class="mobile-filter-facets__facet"
            >
              <search-fieldset
                :title="$t('dictionary.subtitles')"
                :subtitle="filter.subtitles.map(subtitleValue).join(', ')"
                :stand-out="filter.subtitles.length > 0"
              >
                <template #action>
                  <a class="el:o-fieldset-frame__action" href="#">
                    <icon
            ...
            <div
              @click="isFiltering.price = true"
              class="mobile-filter-facets__facet"
            >
              <search-fieldset
                :title="$t('dictionary.price')"
                :subtitle="`$ ${filter.price[0]} - $ ${filter.price[1]}`"
                :stand-out="filter.price[0] > 0 || filter.price[1] < 2500"
              >
                <template #action>
                  <a class="el:o-fieldset-frame__action" href="#">
                    <icon
          ...
          <div
            v-show="isFiltering.root_audio"
            v-bar
            class="mobile-filter-view mobile-filter-options-view"
            key="mobile-filter-options__audio"
          >
            <search-facet-options
              :options="suggestions.root_audio.filtered"
              ...
              "
              @excludeFacetOption="
                $emit('optionRemovedFromFilter', 'root_audio', $event)
              "
            >
            </search-facet-options>
          ...
          <div
            v-show="isFiltering.subtitles"
            v-bar
            class="mobile-filter-view mobile-filter-options-view"
            key="mobile-filter-options__subtitle"
          >
            <search-facet-options
              :options="suggestions.subtitles.filtered"
              ...
              "
              @excludeFacetOption="
                $emit('optionRemovedFromFilter', 'subtitles', $event)
              "
            >
            </search-facet-options>
            <div class="po-a b0 d-f jc-c w100">
          ...
          <div
            v-show="isFiltering.provider_name"
            v-bar
            class="mobile-filter-view mobile-filter-options-view"
            key="mobile-filter-options__provider"
          >
            <search-facet-options
          ...
          <div
            v-show="isFiltering.price"
            class="mobile-filter-view mobile-filter-options-view"
            key="mobile-filter-options__price"
          >
        ...
        <div class="el:o-fieldset-frame__item">
          ...
          <div style="margin-top: 2em; display: flex;">
            ...
            <number-input
              ...
              :min="0"
              :max="2500"
              @change="$emit('priceValueChanged', [filter.price[0], $event])"
            ></number-input>
          </div>
        </div>
      </search-fieldset>
      ...
      <search-fieldset :title="$t('dictionary.audios')" :stand-out="true">
        ...
        <div class="el:o-fieldset-frame__item">
          <search-facet-options-filter
            ...
            @changedInputFacetOptionSuggestion="
              suggestionInputChange('root_audio', ...$event)
            "
          >
          </search-facet-options-filter>
        </div>
        <div class="el:o-fieldset-frame__item" v-bar>
          <search-facet-options
            ...
            :noOptionsMessage="
              this.suggestions.root_audio.current &&
              $t('dictionary.no_suggestions_message', {
      ...
      <search-fieldset :title="$t('dictionary.subtitles')" :stand-out="true">
        ...
        <div class="el:o-fieldset-frame__item">
          <search-facet-options-filter
            ...
            @changedInputFacetOptionSuggestion="
              suggestionInputChange('subtitles', ...$event)
            "
          >
          </search-facet-options-filter>
        </div>
        <div class="el:o-fieldset-frame__item" v-bar>
          <search-facet-options
            ...
            :rootClasses="['scrollable-ul']"
            :noOptionsMessage="
              this.suggestions.subtitles.current &&
              $t('dictionary.no_suggestions_message', {
  ...
  export default {
    ...
    methods: {
      copyAggregationBuckets: function (aggregations) {
        this.suggestions.root_audio.source = aggregations.root_audio.buckets.map(
          ({ key, doc_count }) =>
            Object.assign(
              {},
              { id: key, name: this.audioValue(key), count: doc_count }
            )
        );
        this.suggestions.root_audio.filtered = this.suggestions.root_audio.source;

        this.suggestions.subtitles.source = aggregations.subtitles.buckets.map(
          ({ key, doc_count }) =>
            Object.assign(
              {},
              { id: key, name: this.subtitleValue(key), count: doc_count }
            )
        );
        this.suggestions.subtitles.filtered = this.suggestions.subtitles.source;

        this.suggestions.provider_name.source = aggregations.provider_name.buckets.map(
          ({ key, doc_count }) =>
            Object.assign(
              {},
              { id: key, name: this.providerValue(key), count: doc_count }
            )
        );
