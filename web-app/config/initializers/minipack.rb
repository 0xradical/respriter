...
Minipack.configuration do |c|
  ...
  c.manifest = if Rails.env.development? && ENV['WEBPACK_DEV_SERVER_PUBLIC'].present?
                 "#{ENV['WEBPACK_DEV_SERVER_PROTOCOL']}://#{ENV['WEBPACK_DEV_SERVER_PUBLIC']}/assets/manifest.json"
               else
                 Rails.root.join('public', ENV['WEBPACK_PUBLIC_OUTPUT_PATH'], 'manifest.json')
               end
