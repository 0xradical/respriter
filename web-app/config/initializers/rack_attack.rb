...
class Rack::Attack
  ...
  (1..THROTTLE_LEVELS).each do |level|
    ...
    throttle("#{limit}req/ip/#{period}s", :limit => limit, :period => period) do |req|

      cache_key = "#{Time.now.to_i/period}:#{limit}req/ip/#{period}s:#{req.remote_ip}"
      ...
      unless req.path.start_with?('/assets')
        if req.browser.bot?
          ...
          if inspector.trusted_claim?
            self.cache.write(
              "allow:#{req.remote_ip}", 
              "ua=#{req.browser.name}&raw_ua=#{req.user_agent}&country=#{req.country}", 
        ...
        if (actual_reqs >= limit)
          ...
          if actual_reqs >= max_limit
            self.cache.write(
              "block:#{req.remote_ip}",
              "reqs=#{actual_reqs}&ua=#{req.browser.name}&raw_ua=#{req.user_agent}&country=#{req.country}", 
