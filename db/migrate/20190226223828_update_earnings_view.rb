class UpdateEarningsView < ActiveRecord::Migration[5.2]
 
  def up
    execute "DROP VIEW earnings"
    execute <<-SQL
      CREATE OR REPLACE VIEW earnings AS
      SELECT tracked_actions.id , sale_amount, earnings_amount, ext_click_date, ext_sku_id, ext_sku_name, ext_id,
