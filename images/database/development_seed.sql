...
-- COPY app.orphaned_profiles (id,user_account_id,name,country,short_bio,long_bio,email,website,avatar_url,public_profiles,languages,course_ids,state,slug,claimable_emails,claimable_public_profiles,claim_code,created_at,updated_at) FROM '/app/db/seeds/orphaned_profiles.csv' WITH CSV HEADER DELIMITER ',';
-- COPY app.providers (id,name,description,slug,afn_url_template,published,published_at,created_at,updated_at,encoded_deep_linking,name_dirty,name_changed_at) FROM '/app/db/seeds/providers.csv' WITH CSV HEADER DELIMITER ',';
-- COPY app.courses (id,global_sequence,name,description,slug,url,url_md5,duration_in_hours,price,rating,relevance,region,audio,subtitles,published,stale,category,provider_id,created_at,updated_at,dataset_sequence,resource_sequence,tags,video,source,pace,certificate,pricing_models,offered_by,syllabus,effort,enrollments_count,free_content,paid_content,level,__provider_name__,__source_schema__,instructors,curated_tags,refinement_tags,last_execution_id,schema_version) FROM '/app/db/seeds/courses.csv' WITH CSV HEADER DELIMITER ',';

-- SELECT setval('app.providers_id_seq', MAX(id), true)
-- FROM app.providers;
