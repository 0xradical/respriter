...
run-%: $(LOCAL_ENV_FILES) ## Run a given container attached
	$(DOCKER_COMPOSE) up `$(DOCKER_CONTAINER_ALIAS) $*`

up: $(LOCAL_ENV_FILES) ## Run all containers dettached
	$(DOCKER_COMPOSE) up -d $(SERVICES)

up-%: $(LOCAL_ENV_FILES) LESS_PRIORITY-% ## Run a given container dettached
	$(DOCKER_COMPOSE) up -d `$(DOCKER_CONTAINER_ALIAS) $*`

up-persistence: up-search up-database up-napoleon-database up-s3 ## Run all persistence related containers

down-persistence: down-search down-database down-napoleon-database down-s3 ## Run all persistence related containers

down: clean ## Alias to clean

down-%: $(LOCAL_ENV_FILES) LESS_PRIORITY-% ## Stops a given container
	$(DOCKER_COMPOSE) stop `$(DOCKER_CONTAINER_ALIAS) $*`
	$(DOCKER_COMPOSE) rm -f `$(DOCKER_CONTAINER_ALIAS) $*`
...
restart-%: $(LOCAL_ENV_FILES) LESS_PRIORITY-% ## Restarts a given container
	@$(DOCKER_COMPOSE_VARS) $(MAKE_BIN)/remote_or_dev $* "heroku ps:restart --app `$(MAKE_BIN)/remote_alias $*`" "docker-compose restart `$(DOCKER_CONTAINER_ALIAS) $*`"

console-%: LESS_PRIORITY-% ## Run console for a given app, local or remote
	@$(DOCKER_COMPOSE_VARS) $(MAKE_BIN)/remote_or_dev $* "heroku run console --app `$(MAKE_BIN)/remote_alias $*`" "docker-compose run --rm `$(DOCKER_CONTAINER_ALIAS) $*` make console"
...
bash-%: $(LOCAL_ENV_FILES) LESS_PRIORITY-% ## Runs bash for a given container
	@$(DOCKER_COMPOSE_VARS) $(MAKE_BIN)/remote_or_dev $* "heroku run bash --app `$(MAKE_BIN)/remote_alias $*`" "docker-compose run --rm `$(DOCKER_CONTAINER_ALIAS) $*` /bin/bash"
...
sh-%: $(LOCAL_ENV_FILES) LESS_PRIORITY-% ## Runs sh for a given container
	@$(DOCKER-COMPOSE) run --rm `$(DOCKER_CONTAINER_ALIAS) $*` /bin/sh
...
logs-%: $(LOCAL_ENV_FILES) LESS_PRIORITY-% ## Display and follow dev logs for a given container
	@$(DOCKER_COMPOSE_VARS) $(MAKE_BIN)/remote_or_dev $* "heroku logs -t --app `$(MAKE_BIN)/remote_alias $*`" "docker-compose logs -f `$(DOCKER_CONTAINER_ALIAS) $*`"
...
.PHONY: run run-% up up-% up-persistence down-persistence down down-% clean wipe restart-% console-% psql-% bash bash-% bash-ports-% sh sh-% sh-ports-% detached-make-% attached-make-% logs logs-% logs-remote-% watch watch-%
