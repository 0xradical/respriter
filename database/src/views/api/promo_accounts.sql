...
BEGIN
  ...
  INSERT INTO app.promo_accounts (id, user_account_id, certificate_id, price, purchase_date, order_id, paypal_account, state, state_info)
  VALUES (COALESCE(promo_account.id, public.uuid_generate_v4()), promo_account.user_account_id, cert_id, promo_account.price, promo_account.purchase_date, promo_account.order_id, promo_account.paypal_account, COALESCE(promo_account.state, 'pending'), promo_account.state_info)
  ON CONFLICT ON CONSTRAINT cntr_promo_accounts_user_account_id DO
    UPDATE SET certificate_id = EXCLUDED.certificate_id,
               ...
               state = COALESCE(EXCLUDED.state, app.promo_accounts.state),
               state_info = CASE EXCLUDED.state WHEN 'rejected' THEN COALESCE(EXCLUDED.state_info, app.promo_accounts.state_info) ELSE EXCLUDED.state_info END,
