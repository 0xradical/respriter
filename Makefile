...
APP_IMAGE_NAME := classpert/developers-dashboard
APP_IMAGE_TAG  := $(shell cat package-lock.json | $(SHA1SUM) | sed -e 's/ .*//g')
APP_IMAGE      := $(APP_IMAGE_NAME):$(APP_IMAGE_TAG)

CYPRESS_IMAGE_NAME := classpert/cypress_developer_dashboard
...
GITHUB_ACCESS_TOKEN ?= $(shell heroku config:get GITHUB_ACCESS_TOKEN --app clspt-developers-dashboard-prd)
...
up: ./envs/local.env ## Run containers
	@$(DOCKER_COMPOSE) up -d developer mockserver
