...
<script>
 ...
 export default {
  ...
  methods: {
    ...
    fetchTags () {
      var vm = this
      fetch('/api/user/v1/interests.json', { method: 'GET' }).then(function (resp) {
        resp.json().then(function (json) {
          vm.tags = _.map(json, function (data) {
            return { label: vm.$i18n.t(`tags.${data}`), id: data }
          })
        })
      });
    },
    ...
    addToPreferences (selected) {
      ...
      this.tags.splice(this.tags.indexOf(selected), 1)
    },

    removeTag (tag) {
      this.tags.splice(0,0, { label: this.$i18n.t(`tags.${tag}`), id: tag })
