class CourseReviewMailer < ApplicationMailer
  def build
    ...
    @name = (@profile&.username || @profile&.name&.upcase_first || '').split(/\b+/).first
    @locale = (@user.tracking_data&.fetch('preferred_languages',['en']).map(&:to_sym) & I18n.available_locales).first

    subject = if @name
      choice = SecureRandom.random_number(3) + 1
      I18n.t("emails.course_review.subjects.#{choice}", course: @course, name: @name, locale: @locale)
    else
      I18n.t('emails.course_review.subjects.1', course: @course, locale: @locale)
    end
